...
const sidebarList = [
  ...
  {imgSrc: <MdAutoGraph className="w-5 h-5"/>, title: "Markets", url: "/dashboard"},
...
export default function Sidebar() {
  ...
  return (
      ...
      {sidebarList.map((item, index) => (
          <Link to={item.url}><div className={"flex flex-row gap-4 items-center justify-start cursor-pointer " + (activeItem != index ? "text-[#9FA7B1] hover:text-[#30B778]" : "text-[#30B778]")}>
              <div>{item.imgSrc}</div>
              <div className="text-left w-0 overflow-hidden transition-all duration-500 group-hover:w-[110px]">{item.title}</div>
          </div></Link>
      ))}
      <Link to="/"><div className="absolute group-hover:w-[152px] w-[70px] bottom-10 left-5 flex flex-row gap-3 items-center justify-start cursor-pointer text-[#9FA7B1] hover:text-[#30B778]"><MdLogout className="w-5 h-5 "/><p className="w-0 overflow-hidden group-hover:w-[70px] transition-all duration-500">LogOut</p></div></Link>
  </div>
  )
}
